...
from cct_module import calculate_cct
import io
import base64
        ...
        dbc.Tab(label="Calculations", children=[
            dbc.Row([
                dbc.Col(width=4, children=[  # Half the screen width for settings, reference, and spectral data
                    ...
                    # dbc.Card([
                    #     dbc.CardHeader("Upload CSV", style={'backgroundColor': '#000000', 'color': '#BA9E5E'}),
                    #     dbc.CardBody([
                    #         dcc.Upload(
                    #             id='upload-data',
                    #             children=html.Div([
                    ...
                    #         ])
                    # ], id='upload-card', style={'margin-top': '20px'}),
                    dbc.Card([
...
def update_spectra_test_table(test_choice):
    if test_choice == 'Warm LED':
        ...
        return [], []  # Return empty data and columns if no valid choice
...
def update_all_outputs(rows, columns, test_choice, ref_choice, stored_cct_value, n_clicks, ref_cct_value):
    fig = go.Figure()
    ...
    # Debug: print the DataFrame to verify data integrity
    print("test_df head:\n", test_df.head())
    print("test_df dtypes:\n", test_df.dtypes)

    # Ensure the DataFrame has the expected columns and types
    # if 'wavelength' not in test_df.columns or 'intensity' not in test_df.columns:
    #     return fig, html.Div("Invalid data format."), "Spectral Similarity Index: N/A"
    # if not pd.api.types.is_numeric_dtype(test_df['wavelength']) or not pd.api.types.is_numeric_dtype(test_df['intensity']):
    #     return fig, html.Div("Data must be numeric."), "Spectral Similarity Index: N/A"
    # if test_df['wavelength'].isnull().any() or test_df['intensity'].isnull().any():
    #     return fig, html.Div("Data contains null values."), "Spectral Similarity Index: N/A"

    # Plot test spectrum
    if test_choice == 'Custom':
        fig.add_trace(go.Scatter(x=test_df['wavelength'], y=test_df['intensity'], mode='lines', name='Test Spectrum'))
    else:
        fig.add_trace(go.Scatter(x=test_df['wavelength'], y=test_df['intensity'], mode='lines', name=f'Test Spectrum [CCT: {CCT_MAPPING[test_choice]}]'))
    ...
    fig.add_trace(go.Scatter(x=df['wavelength'], y=df['intensity'], mode='lines', name=f'Reference Spectrum [CCT: {int(graph_cct_var)}]'))
